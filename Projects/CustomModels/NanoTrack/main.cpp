...
public:
	...
	CVI_RC infer_head()
	{
		...
		// size_t count = b127_output_tensors->count; // Number of elements (not bytes)
		...
		// size_t count_ = b255_output_tensors->count; // Number of elements (not bytes)

		// MMMConvertFp32ToBf16(src_fp32_, dst_bf16_, count_);

		
		head_input_tensors[0].sys_mem = (uint8_t *)malloc(b127_output_tensors->mem_size);
		head_input_tensors[1].sys_mem = (uint8_t *)malloc(b255_output_tensors->mem_size);

		memcpy(head_input_tensors[0].sys_mem, b127_output_tensors->sys_mem, b127_output_tensors->mem_size);
		memcpy(head_input_tensors[1].sys_mem, b255_output_tensors->sys_mem, b255_output_tensors->mem_size);
		ret = CVI_NN_Forward(head_model, head_input_tensors, head_input_num, head_output_tensors, head_output_num);
